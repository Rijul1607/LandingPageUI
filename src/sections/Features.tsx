...
const FeatureTab=(props:typeof tabs[number] & ComponentPropsWithoutRef<'div'> & {selected:boolean})=>{
  ...
  useEffect(()=>{
    ...
    const circum=height*2+width*2;

    const times=[0,width/circum ,(width+height)/circum, (width*2+height)/circum,1]
    ...
    animate(xper,[0,100,100,0,0],
      options)
    animate(yper,[0,0,100,100,0],
      options
    )
  },[props.selected]);
  ...
  return(
    <div ref={tabRef} onClick={props.onClick}  onMouseEnter={handleTabHover} key ={props.title} className="border border-white/15 flex p-2.5 rounded-xl gap-2.5 items-center lg:flex-1 relative">
      {props.selected && (

      <motion.div style={{
        maskImage,
      }} className="absolute inset-0 -m-px border border-[#A369FF] rounded-xl [mask-image:]"></motion.div>
      )}
